...
- Do not try to re-upload `0.1.0`. PyPI files are immutable by version; the plugin repository release train starts at `0.2.1`.
...
- The Claude path splits into two surfaces:
  - Claude official plugin directory: `.claude-plugin/marketplace.json` plus `.claude-plugin/plugin.json`, then submit through Anthropic's Console or Claude.ai directory form.
...
```bash
cyberhuatuo install-command --username <maintainer-github> --framework langchain --release-tag v0.2.5 --target-contributors 3
cyberhuatuo market-ready --no-remote
cyberhuatuo launch-assets --username <maintainer-github> --framework langchain --release-tag v0.2.5 --target-contributors 3
cyberhuatuo proof-pack --username <maintainer-github> --framework langchain --release-tag v0.2.5 --target-contributors 3
python -m cyberhuatuo candidate-install-smoke --username <maintainer-github> --framework langchain --release-tag v0.2.5 --target-contributors 3
cyberhuatuo first-invite --username <maintainer-github> --invitee <external-contributor-github> --framework langchain --release-tag v0.2.5 --target-contributors 3 --source-url <created Growth Issue URL>
...
```bash
cyberhuatuo market-ready --remote --strict-remote --username <maintainer-github> --framework langchain --release-tag v0.2.5 --target-contributors 3
python scripts/check_marketplace_release.py --remote --strict-remote --username <maintainer-github> --framework langchain --release-tag v0.2.5 --target-contributors 3
```
...
11. Start the public cold-start campaign with an explicit target count:
   ...
   ```bash
   cyberhuatuo launch-campaign --username <maintainer-github> --framework langchain --release-tag v0.2.5 --target-contributors 3 --surface "PyPI release"
   ```
...
12. Check public traction proof before claiming breakout:
   ...
   ```bash
   cyberhuatuo record-market --username <maintainer-github> --framework langchain --channel pypi --status submitted --submission-url <reviewable public URL> --release-tag v0.2.5
   cyberhuatuo market-status --username <maintainer-github> --framework langchain --release-tag v0.2.5
   ```
   ...
   ```bash
   cyberhuatuo traction-proof --username <maintainer-github> --framework langchain --release-tag v0.2.5 --target-contributors 3
   ```
...
12. Record public traction velocity only after a real market check:
   ...
   ```bash
   cyberhuatuo traction-proof --username <maintainer-github> --framework langchain --release-tag v0.2.5 --target-contributors 3 --record-snapshot
   ```
...
Goal: make the stdio Server discoverable through the official MCP Registry without relying on repository search.

Release contract:

- Registry name: `io.github.JinNing6/cyberhuatuo`
- Metadata: `server.json`
- Package registry: PyPI
- Package identifier: `cyberhuatuo`
- Runtime: `uvx cyberhuatuo mcp`
- Ownership proof in the PyPI long description: `mcp-name: io.github.JinNing6/cyberhuatuo`

The Registry validates the ownership marker against the README of the exact published PyPI package. Therefore `server.json` for `0.2.5` must not be published before PyPI serves `cyberhuatuo==0.2.5` with the matching marker.
...
After publication, verify the actual Registry API result:

```bash
curl "https://registry.modelcontextprotocol.io/v0.1/servers?search=io.github.JinNing6/cyberhuatuo"
```

The Registry metadata for a published version is immutable. Any metadata correction must use a new unique server version; do not overwrite or reuse `0.2.5`.
...
Claude official plugin-directory submission:
...
2. Submit through one of Anthropic's official forms:
   - Console for individual authors or Console organizations: `https://platform.claude.com/plugins/submit`
   - Claude.ai Team/Enterprise with directory-management access: `https://claude.ai/admin-settings/directory/submissions/plugins/new`
3. Treat the submission as pending until Anthropic accepts it. The accepted directory is surfaced in Claude Code as `claude-plugins-official`; submission does not imply an `Anthropic Verified` badge.
...
5. Test after approval:
   ...
   claude plugin install cyberhuatuo-plugin@claude-plugins-official
   ```
...
6. Generate the launch scroll:
   ...
   ```bash
   cyberhuatuo launch --username <maintainer-github> --framework langchain --release-tag v0.2.5
   ```

7. Generate the Soul Ring Bounty Board before public posts so external users see real claimable coverage gaps:

   ```bash
   cyberhuatuo bounty --username <maintainer-github> --framework auto --top-n 8 --release-tag v0.2.5 --target-contributors 3
   ```
...
8. Generate the launch campaign with a target first-ring contributor count:
   ...
   ```bash
   cyberhuatuo launch-campaign --username <maintainer-github> --framework langchain --release-tag v0.2.5 --target-contributors 3
   ```
...
20. Submit the Claude plugin to Anthropic's official directory after `claude plugin validate .`; treat it as pending until accepted into `claude-plugins-official`.
...
- Claude Code plugin marketplaces are Git-backed catalogs; third-party plugins can be submitted to Anthropic's official directory through the Console or Claude.ai forms after `claude plugin validate`.
