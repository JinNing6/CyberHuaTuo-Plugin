...
最轻的入口不需要模型 Key，也不会启动向量库：安装一次，粘贴报错，直接得到经过复核的病灶、精确药方、验证命令、风险/回退和证据。

```bash
python -m pip install --upgrade "cyberhuatuo>=0.2.4"
...
`cure` **优先返回 Gold**，而且绝不自动执行修复。Gold 错误签名未命中时，最多回退一张高置信 **Reviewed 候选**，并明确标注“验证后再用”；需要严格只查 Gold 时传 `--gold-only`。需要更广泛的检索和可选 LLM 分析时，再使用 `diagnose`。

Agent 原生入口仍然保留：把赛博华佗 MCP 接进编码 Agent，遇到报错后由 Agent 调用 `diagnose` 自救。修好后，`save_prescription` 先保存为本地 `draft`；`upload_prescription` 提交公开复现与审核。只有审核接受的 `reviewed` 或 `gold` 药方才计入魂环。
...
首张 Gold 药方已在隔离环境用 `langchain==1.3.13` 与 `langchain-openai==1.3.5` 完成真实验证。配置模型 Key 后，可以继续使用更深度的 AI 望闻问切。

药方现在也有稳定“科室”分类。先用 `cyberhuatuo departments` 查看科室，再通过正常的 Gold 优先路径查阅维护者历史踩坑形成的 Reviewed 候选：
...
cyberhuatuo cure-feedback general-ai-jsonl-bom-001 partial \
  --verification "首条记录已保留，但下游校验仍失败" \
  --verification-method targeted-parser-test
```

反馈命令只在本地写入绑定病例 ID 的 JSONL 事件，不保存原始 traceback 或查询。科室过滤不会改变信任标签：Gold 是已验证药方，Reviewed 是验证后再用的候选，Draft 仅供参考。
...
如果真实判定与预期不一致，可以在不执行、不上传命令的前提下生成本地脱敏病例：

```bash
cyberhuatuo guard "Remove-Item C:\\ -Recurse -Force" --workspace-root . --expected BLOCK --report reports/guard-report.md
```

CLI 会先展示完整脱敏预览，再询问是否写入；默认拒绝覆盖已有报告，不发起网络请求，也不会自动创建公开 Issue。私有项目名等业务标识可重复使用 `--redact <literal>`。非交互环境只有在检查预览后才能显式增加 `--confirm-report`。使用 `--expected` 时，匹配返回 `0`、不匹配返回 `1`；报告生成或写入失败固定返回 `4`，不会被 `--exit-zero` 掩盖。

公开且已脱敏的普通病例分别进入 [Guard 误报](https://github.com/JinNing6/CyberHuaTuo-Plugin/issues/new?template=guard-false-positive.yml)、[普通 Guard 漏报](https://github.com/JinNing6/CyberHuaTuo-Plugin/issues/new?template=guard-false-negative.yml)或[集成缺口](https://github.com/JinNing6/CyberHuaTuo-Plugin/issues/new?template=guard-integration-gap.yml)。可稳定利用的 Hook、解析器、包装器或协议绕过必须进入[私密漏洞报告](https://github.com/JinNing6/CyberHuaTuo-Plugin/security/advisories/new)，不得先公开复现细节。
...
cyberhuatuo traction-proof --username your-github-username --framework langchain --release-tag v0.2.4 --target-contributors 3 --record-snapshot
cyberhuatuo install-command --username your-github-username --framework langchain --release-tag v0.2.4 --target-contributors 3
python -m cyberhuatuo candidate-install-smoke --username your-github-username --framework langchain --release-tag v0.2.4 --target-contributors 3
cyberhuatuo first-invite --username your-github-username --invitee external-contributor-github-username --framework langchain --release-tag v0.2.4 --target-contributors 3 --source-url https://github.com/JinNing6/CyberHuaTuo-Plugin/issues/123
cyberhuatuo market-ready --no-remote
cyberhuatuo market-ready --remote --strict-remote --username your-github-username --framework langchain --release-tag v0.2.4 --target-contributors 3
cyberhuatuo proof-pack --username your-github-username --framework langchain --release-tag v0.2.4 --target-contributors 3
cyberhuatuo market-copy --username your-github-username --framework langchain --release-tag v0.2.4 --target-contributors 3
...
```bash
python -m pip install --upgrade "cyberhuatuo @ git+https://github.com/JinNing6/CyberHuaTuo-Plugin.git@v0.2.4"
python -m pip install --upgrade cyberhuatuo  # after PyPI latest matches v0.2.4
```
