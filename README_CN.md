...
Registry 客户端也可以使用与 PyPI 包名一致的 MCP 入口：
...
它与 `cyberhuatuo-mcp` 启动同一个 stdio Server。官方 Registry 标识为 `io.github.JinNing6/cyberhuatuo`；根目录 `server.json` 固定了 PyPI 包、精确版本、`uvx` 运行时和 `mcp` 参数。
...
cyberhuatuo install-command --username your-github-username --framework langchain --release-tag v0.2.5 --target-contributors 3
...
cyberhuatuo market-ready --remote --strict-remote --username your-github-username --framework langchain --release-tag v0.2.5 --target-contributors 3
cyberhuatuo proof-pack --username your-github-username --framework langchain --release-tag v0.2.5 --target-contributors 3
...
- **Claude markets**：通过 `.claude-plugin/plugin.json` 和 `.claude-plugin/marketplace.json` 支持 Claude Code，`claude plugin validate .` 通过后提交 Anthropic 官方插件目录；同时通过 `claude-desktop/manifest.json` 与 `.github/workflows/package-claude-mcpb.yml` 打包 Claude Desktop MCPB，用于 Connectors Directory 审核。
