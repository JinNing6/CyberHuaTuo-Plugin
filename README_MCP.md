...
同一个 Server 也提供与 PyPI 包名一致的 Registry 入口：

```bash
uvx cyberhuatuo mcp
```

根目录 [`server.json`](server.json) 将 `io.github.JinNing6/cyberhuatuo` 映射到 PyPI `cyberhuatuo`，并通过固定的 `mcp` 参数启动 stdio Server。Registry 发布前必须先确认对应 PyPI 版本的 README 已包含相同的 `mcp-name` 所有权标识。
...
cyberhuatuo launch-assets --username your-github-username --framework langchain --release-tag v0.2.5 --target-contributors 3
...
python scripts/check_marketplace_release.py --remote --strict-remote --username your-github-username --framework langchain --release-tag v0.2.5 --target-contributors 3
```
